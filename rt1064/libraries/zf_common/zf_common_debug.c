...
extern int lock_box;
extern int track_error;  // 存储计算得到的误差
void debug_interrupr_handler (void)
{
    ...
    if(zf_debug_init_flag)
    {
		...
		uint8 fifo_get_data[64] = {0}; 
		fifo_data_count = fifo_used(&debug_uart_fifo); 
		//printf("START:%d\n",fifo_data_count);
		if(fifo_data_count ==1)                                                // 读取到数据了
        {
			...
			//printf("length %d ",fifo_data_count);
			//for(int i=0;i<1;i++)printf("%c ",fifo_get_data[i]);//printf("\n");
			index = fifo_get_data[0];
//			if(fifo_get_data[0]=='W')moveflag = 1;
//			if(fifo_get_data[0]=='A')moveflag = 3;
//			if(fifo_get_data[0]=='D')moveflag = 4;
//			if(fifo_get_data[0]=='S')moveflag = 2;
//			if(fifo_get_data[0]=='p')resetflag = 1;
//			if(fifo_get_data[0]=='g')resetflag = 2;
//			if(fifo_get_data[0]=='r')resetflag = 3;
//			if(resetflag == 1)p_Odometer_Init();
//			if(resetflag == 2)g_Odometer_Init();
//			if(resetflag == 3)roundabout_entry_flag = 0;
//			resetflag = 0;
//			//printf("moveflag:%d %d %4f %4f\n",moveflag,roundabout_entry_flag,p_odometer.distance,g_odometer.distance);
//			printf("z_angle: %5f %5f %4d %4d %4d\n", g_odometer.distance,p_odometer.distance,crossroad_flag,track_error,moveflag);
			fifo_clear(&debug_uart_fifo);
		}
